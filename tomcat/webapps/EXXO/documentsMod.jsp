...
<html>
    ...
    <body id="documentsMod_jsp">
                            ...
                            <td>
                                ...
                                <div id="foot">
                                    <ul class="topMenu">
                                        ...
                                        <li><a href="docClassic.jsp<% if (owner != null) {%>?owner=<%=owner%><%}%>" class="footmenu">Sonlandır</a></li>
                        ...
                        <td id="menu-td" class="menu-td" valign="top">
                            ...
                            <div id="counts" class="exxo-shadow">
                                <table>
                                    ...
                                    <% if (owner == null || !owner.equals("-100")) {%>
                                    <tr><td>boş yer:</td><td class="res"><%=free%></td></tr>
                                            <% }%>
                                </table>
                            </div>
                            <% if (owner != null && owner.equals("-101")) {%>
                            <a href="LoadInnerDocs" id="load">Məktubdan yükləyin</a>
                            <%}%>
                        </td>
        ...
        <div class="menu1 aui-overlaycontext-hidden exxo-shadow">
            <ul><li class="item-add c "><a >Kataloq əlavə et</a></li>
                <li class="item-move np"><a >Sürüşdür</a></li>
                <li class="item-kill np"><a >Sil</a></li>
                <li class="file-ver f np"><a >НYeni versiya</a></li>
                <li class="item-change np" id="item-change"><a >Adını dəyişdir</a></li>
                <li class="file-change f np" id="file-change"><a >Məlumatı dəyişdir</a></li>
                <li class="file-add c " id="file-add"><a >Faylı saxla</a></li>
                <li class="files-add c " id="files-add"><a >Faylları saxla</a></li>
                <li class="file-prop f " id="file-prop"><a >Xassələri</a></li>
            </ul></div>
        <div id="form"><div class="close" id="close"><span>BAĞLA</span></div>
            <form id="FILEFORM" action="addFile" enctype="multipart/form-data" method="POST">
                <label for="file">Faylı seç</label><input type="file" name="file" id="file">
                <label for="name">Faylın adı</label><input type="text" name="name" id="name">
                <label for="dopusk">Допуск</label>
                <select name="dopusk" id="dopusk" <% if (general) { %>disabled="disabled"<% }%>>
                    <option value="0" <% if (owner != null && owner.equals("-100")) { %>selected<% }%>>ümumi</option>    
                    <option value="1" >proyekt üçün</option>
                    <option value="2" >qrup üçün</option>
                    <option value="3" >bölünmı üçün</option>
                    <option value="4" <% if (!general) { %>selected<% }%>>şəxsi</option>
                </select>
                <label for="discription">Açıqlama</label><textarea name="discription" rows="5" cols="22" ></textarea>
                <label for="tags">Teq</label><textarea name="tags" rows="5" cols="22" ></textarea><br><br>
                <% if (owner != null) {%><input type="hidden" name="owner" value="<%= owner%>"><%}%>
                <input type="submit" value="Yadda saxla"></form>
        </div>
        <div id="form1"><div class="close" id="close"><span>BAĞLA</span></div>
            <form id="FILEFORM1" action="fileVer" enctype="multipart/form-data" method="POST">
                <label for="file">Fayl seçin</label><input type="file" name="file" id="file">
                <% if (owner != null) {%><input type="hidden" name="owner" value="<%= owner%>"><%}%>
                <input type="submit" value="Yadda saxla"></form>
        </div>
        <div id="hic"><div class="close" id="closehic"><span>BAĞLA</span></div><div id="mhic"></div></div>
        <div id="moveMain">
            <div class="close"><span>BAĞLA</span></div>
        ...
        <script type="text/javascript">
            ...
            var name = window.prompt("fayl və ya kataloqa yeni ad verin", html.text());
            ...
            if (node.hasClass("copy")) alert ("Sənədin surətinə yeni versiya əlavə edə bilməzsiniz!");
            else if (node.hasClass("s_6")) alert ("Əməliyyat təsdiq edilmiş sənədlə qadağandır!");
            else {
            var id = node.get("id");
