<%@ page contentType="text/html" pageEncoding="UTF-8"%><jsp:useBean id="SLOGAN" class="BEANS.SLOGAN" scope="application"/><jsp:useBean id="CSS" class="BEANS.CSSTotal" scope="application"/><jsp:useBean id="CSS3" class="BEANS.css1Bean" scope="page"/><jsp:useBean id="NP" class="BEANS.photosBean" scope="page"/><%
...
<html>
    <head>
        ...
        <style type="text/css">
            ...
            #content {position: relative;margin-top: 0px; margin-left: 10px; width:auto;height: auto;margin-bottom: 10px;background-color: white;padding: 4px;overflow:visible; width: 95%; border-color:#DEDEDE #DEDEDE #BFBFBF #DEDEDE; border-width: 1px 1px 1px 1px; border-style: solid; text-align: center;}
            ...
            #menuList a {background-image: url('small/photo_album.png');background-position: center left;background-repeat: no-repeat; padding: 2px; padding-left: 22px;}
        </style>
        <style>
            <%=CSS.getCSS(request)%>
        </style>
        <style>
            <%=CSS3.getCSS("photos.jsp", request)%>
        </style>
    </head>
    <body id="photos_jsp">
                    ...
                    <tr id="tr">
                        <td id="menu-td" class="menu-td" valign="top">
                            ...
                            <div id="counts" class="exxo-shadow">
                                <%if (owner != null) {%> <div id="author"><%=NP.fio%></div><% }%>
                                <table>
                                    <tr><td>Альбомов:</td><td class="res"><%=NP.c%></td></tr>
                                    <tr><td>Фотографий</td><td class="res"><%= NP.v%></td></tr>
                                    <tr><td>Просмотров:</td><td class="res"><%= NP.count%></td></tr>
                                    <tr><td>Понравилось:</td><td class="res"><%= NP.respcount%></td></tr>
                                    <tr><td>Комментариев</td><td class="res"><%= NP.commcount%></td></tr>
                                </table>
                            </div>
                            <div id="menuList">
                                <a href="galClassic.jsp<%=own%>">Фотоальбомы</a>
                            </div>
                        </td>
